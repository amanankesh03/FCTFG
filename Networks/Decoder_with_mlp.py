...
class Decoder(nn.Module):
    def __init__(self, opts):
        ...
        blur_kernel=[1, 3, 3, 1]
        self.n_mlp = opts.decoder_n_mlp
        self.lr_mlp = opts.decoder_lr_mlp    
    ...
    def forward(
        ...
        # print(f'\n latent shape, {styles.shape}')
        if not input_is_latent:
            styles_list = []
            for s in styles:
                styles_list.append(self.style(s))
               
        styles = torch.stack(styles_list, dim=0)
        if noise is None:
            if randomize_noise:
...
if __name__ =="__main__":
    ...
    print(d.num_layers)
    z_f = torch.randn(10, 16, 512).to(opts.device)
    img, _ = d(z_f)
    print(img.shape)
